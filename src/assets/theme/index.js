const GRAY9 = "#191919"
const GRAY8 = "#2D2D2D"
const GRAY7 = "#404040"
const GRAY6 = "#868e96"
const GRAY5 = "#adb5bd"
const GRAY4 = "#ced4da"
const GRAY3 = "#dee2e6"
const GRAY2 = "#e9ecef"
const GRAY1 = "#f1f3f5"
const GRAY0 = "#f8f9fa"
const GRAY11 = "#2989e8"
const GRAY12 = "#71aeea"

export const light = {
  name: "light",
  colors: {
    color1: "#ffc400",
    color2: "#ff0069",
    color3: "#c00fdc",
    bodyBackground: "#ffffff",
    text: GRAY9,
    secondHeadLine: "#eaecef",
    secondaryText: "#333232",
    tertiaryText: GRAY6,
    mutedText: GRAY5,
    hoveredLinkText: GRAY0,
    border: GRAY4,
    activatedBorder: GRAY6,
    background: GRAY1,
    titleShadow: "rgb(206,177,236)",
    icon: GRAY6,
    divider: GRAY2,
    headerBackground: "rgba(235,235,236,0.85)",
    headerShadow: "rgba(54,53,53,0.08)",
    inlineCodeBackground: GRAY2,
    inlineCodeBackgroundDarker: GRAY4,
    tagBackground: "rgb(238,232,252)",
    selectedTagBackground: GRAY7,
    hoveredTagBackground: "rgba(203,180,238,0.97)",
    hoveredSelectedTagBackground: GRAY8,
    nextPostButtonBackground: "rgba(0, 0, 0, 0.06)",
    hoveredNextPostButtonBackground: "rgba(0, 0, 0, 0.08)",
    seriesBackground: GRAY1,
    tagText: GRAY7,
    selectedTagText: GRAY0,
    tocText: "#939292",
    spinner: GRAY7,
    scrollTrack: GRAY1,
    scrollHandle: GRAY4,
    blockQuoteBorder: GRAY4,
    blockQuoteBackground: GRAY1,
    textFieldBorder: GRAY4,
    textFieldActivatedBorder: GRAY5,
    tableBackground: GRAY1,
    linkText: GRAY11
  },
}

export const dark = {
  name: "dark",
  colors: {
    bodyBackground: GRAY9,
    text: GRAY0,
    secondaryText: GRAY4,
    tocText: "#6c6c6c",
    tertiaryText: GRAY5,
    mutedText: GRAY6,
    hoveredLinkText: GRAY9,
    border: GRAY5,
    activatedBorder: GRAY3,
    background: GRAY8,
    icon: GRAY5,
    divider: GRAY8,
    secondHeadLine: "#2f2f2f",
    headerBackground: "rgba(35,35,35,0.85)",
    headerShadow: "rgba(225,224,224,0.08)",
    inlineCodeBackground: GRAY8,
    titleShadow: "rgb(80,4,169)",
    inlineCodeBackgroundDarker: GRAY7,
    tagBackground: "rgba(72,24,168,0.97)",
    selectedTagBackground: GRAY2,
    hoveredTagBackground: "rgba(184,153,248,0.97)",
    hoveredSelectedTagBackground: GRAY1,
    nextPostButtonBackground: "rgba(255, 255, 255, 0.05)",
    hoveredNextPostButtonBackground: "rgba(255, 255, 255, 0.08)",
    seriesBackground: GRAY8,
    tagText: GRAY2,
    selectedTagText: GRAY9,
    spinner: GRAY1,
    scrollTrack: GRAY8,
    scrollHandle: GRAY7,
    blockQuoteBorder: GRAY7,
    blockQuoteBackground: GRAY8,
    textFieldBorder: GRAY7,
    textFieldActivatedBorder: GRAY6,
    tableBackground: "#292e33",
    linkText: GRAY12
  },
}
